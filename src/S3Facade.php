...
class S3Facade {
	...
	private $resourceMetaFactory;

	public function __construct(array $config, IModifiers $modifiers, IResourceMetaFactory $resourceMetaFactory,
								IImageFactory $imageFactory) {
		$this->config = $config;
		$this->modifiers = $modifiers;
		$this->resourceMetaFactory = $resourceMetaFactory;
	...
	public function getDefaultImage(IResource $resource): ?string {
		$this->resourceMetaFactory->create($resource);
	...
	public function save(ITransferResource $resource, bool $forceModify = false): IFileResource {
		try {
			$meta = $this->resourceMetaFactory->create($resource);
			$meta->modify($image = $this->imageFactory->createFromFile($resource->getLocation()));
	...
	public function link(IFileResource $resource): ?string {
		if ($link = $this->getLink($resource)) {
			return $link;
		}
		$meta = $this->resourceMetaFactory->create($resource);
		$originalLink = $this->getLink($resource->getOriginal());
		if (!$meta->toModify() || !is_string($originalLink)) {
			return null;
		}
